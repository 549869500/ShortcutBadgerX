...
public final class ShortcutBadger {
   ...
   * {@link ShortcutBadgeException} if it fails
   *
   * @param context    Caller context
   * @param badgeCount Desired badge count
   */
  ...
  /**
   * Whether this platform launcher supports shortcut badges. Doing this check
   * causes the side
   * effect of resetting the counter if it's supported, so this method should be
   * followed by
   * a call that actually sets the counter to the desired value, if the counter is
   * supported.
   */
  public static boolean isBadgeCounterSupported(Context context) {
    // Checking outside synchronized block to avoid synchronization in the common
    // case (flag
  ...
  // Initialize Badger if a launcher is availalble (eg. set as default on the
  // device)
  // Returns true if a launcher is available, in this case, the Badger will be set
  // and sShortcutBadger will be non null.
  private static boolean initBadger(Context context) {
    ...
    // Turns out framework does not guarantee to put DEFAULT Activity on top of the
    // list.
